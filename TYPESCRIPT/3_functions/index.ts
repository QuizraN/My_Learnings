...
export default (() => {
  ...
  function getPointsFor(letter:string) {
    ...
    return lettersAndPoints.reduce((computedScore:number, pointsTuple:(string|number)[]) => {
      ...
      if ((letters as string).split('').find((ll) => {ll === letter})) {
  ...
  function layEggs(quantity:number=10, color:string="brown"):void {
  ...
  function pushToCollection<Type>(item:Type, collection:Type[]) {
...
//   function pushToCollection(item:(string|number), collection:any) {
